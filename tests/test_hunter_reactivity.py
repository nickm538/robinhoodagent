...
from rh_agent.data.universe import Candidate, build_universe
...
def test_entry_no_trade_band_allows_immediate_new_buys():
    ...
    targets = [TargetPosition("SNIPE", 0.20, 70.0, 100.0, 2.0, "Tech")]
...
def test_intraday_prescreen_prefers_runners_over_slow_momentum():
    cfg = load_config()
    cfg.raw["universe"]["prescreen"] = {"enabled": True, "max_candidates": 2}
    ...
    class _MD:
        ...
        def get_prices(self, t):
            return _prices(10.0 if t == "RUN" else 30.0)
...
def test_rotation_sells_stale_hold_for_stronger_runner():
    ...
    scan = ScanResult(
        ...
        verdicts=[held_v],
        eligible=[],
        targets=[TargetPosition("NEW", 0.25, 65.0, 125.0, 2.5, "Tech")],
        equity=500.0,
        universe_size=2,
        scored_size=2,
    )
